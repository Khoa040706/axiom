...
const PIT_BRACKETS = [
  ...
  { bracket: "Bậc 2", range: "10 – 30 triệu",  rate: "10%", formula: "0.1×TNTT − 500K" },
    ...
    <div style={{ padding: "28px 28px 40px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 14, marginBottom: 22 }}>
        {[
          ...
          { icon: <TrendingUp size={18}/>, label: vi ? "Giảm trừ bản thân" : "Self deduction", value: fmtM(draft.selfDeduction), accent: "#10B981", note: vi ? "/người/tháng" : "/person/month" },
              ...
              <Section title={vi ? "Giảm trừ gia cảnh (GTGC)" : "Personal Income Tax Deductions"} icon={<Percent size={15}/>} th={th}>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                  <NumField
                    ...
                    note={fmtM(draft.selfDeduction) + (vi?" /tháng":" /month")} th={th}/>
                  <NumField
                    ...
                    note={fmtM(draft.depDeduction) + (vi?" /người/tháng":" /person/month")} th={th}/>
                </div>
                <div style={{ marginTop: 16, background: th.tableHead, borderRadius: 12, padding: "14px 16px" }}>
                  ...
                  {[1, 2, 3].map(n => (
                    <div key={n} style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: th.text2, marginBottom: 4 }}>
                      ...
                      <span style={{ fontWeight: 700, color: "#10B981" }}>{fmtM(draft.selfDeduction + n * draft.depDeduction)}{vi?"/tháng":"/mo"}</span>
                    </div>
                  ))}
                </div>
              </Section>

              <Section title={vi ? "Biểu thuế TNCN — 5 Bậc Lũy Tiến (Cố định theo Luật)" : "Progressive PIT Brackets (Fixed by Law)"} icon={<BookOpen size={15}/>} th={th}>
                <div style={{ marginBottom: 10, fontSize: 12.5, color: dark ? "#93C5FD" : "#1D4ED8", background: dark ? "rgba(59,130,246,0.1)" : "#EFF6FF", padding: "10px 14px", borderRadius: 10, display: "flex", gap: 7, border: "1px solid #BFDBFE" }}>
                  ...
                  <span>{vi ? "Biểu thuế lũy tiến 5 bậc là quy định nhà nước, KHÔNG thể chỉnh sửa." : "The 5-bracket progressive PIT is fixed by Vietnamese law and cannot be modified."}</span>
                </div>
                  ...
                  <tbody>
                    {PIT_BRACKETS.map((b, i) => (
                      <tr key={i} style={{ transition: "background .1s" }} onMouseEnter={e => (e.currentTarget.style.background = dark ? "rgba(255,255,255,0.03)" : "#FAFAFA")} onMouseLeave={e => (e.currentTarget.style.background = "transparent")}>
                        <td style={{ padding: "9px 12px", fontSize: 13, color: th.text1, borderBottom: `1px solid ${th.tableBorder}`, fontWeight: 600 }}>{b.bracket}</td>
                        <td style={{ padding: "9px 12px", fontSize: 13, color: th.text2, borderBottom: `1px solid ${th.tableBorder}` }}>{b.range}</td>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <div style={{ background: th.cardBg, borderRadius: 16, padding: "18px", border: `1px solid ${th.cardBorder}`, boxShadow: "0 2px 8px rgba(0,0,0,0.05)" }}>
            ...
            <div style={{ marginTop: 12, fontSize: 12, color: th.text2, lineHeight: 1.7 }}>
              ...
              <div>• {vi ? `Giảm trừ bản thân: ${fmtM(draft.selfDeduction)}/tháng` : `Self deduction: ${fmtM(draft.selfDeduction)}/mo`}</div>
            </div>
          </div>
